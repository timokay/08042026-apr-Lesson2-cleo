...
| US-053 | Как Plus пользователь, хочу продлить план нажав "Продлить", чтобы не потерять доступ | Should |
| US-054 | Как plus пользователь, хочу делать безлимитные ростеры, чтобы получать максимум ценности | Must |
| US-055 | Как expired plus пользователь, хочу быть автоматически возвращён на free, чтобы не платить за что не использую | Must |
...
### US-053 (Продление)
```gherkin
Given Plus пользователь на /settings
When нажимает "Продлить"
Then redirect → /upgrade с pre-filled plan selection
And после оплаты plan_expires_at продлевается на 30 дней от текущего (не от истечения)

Given план истекает через ≤ 5 дней
When пользователь на /settings
Then показывается badge "Истекает через N дней"
```
...
### US-055 (Auto-downgrade)
```gherkin
Given plan_expires_at = прошлое время (UTC)
When пользователь делает запрос на ростер
Then profile.plan автоматически = 'free', plan_expires_at = NULL
And monthly limit применяется как для free пользователя
And пользователь получает 429 если уже использовал 1 ростер в этом месяце
```
